...
FALLBACK_CHAINS: dict[str, list[str]] = {
    ...
    # US: Western public and key-gated sources first; the China-hosted
    # mirrors (sina, eastmoney, akshare) stay reachable but only as the tail,
    # so a US-only user never sends ticker queries abroad unless every other
    # source has failed.
    "us_equity": ["yahoo", "stooq", "yfinance", "tiingo", "fmp", "finnhub", "alphavantage", "longbridge", "sina", "eastmoney", "akshare", "local"],
    ...
    "forex":     ["mt5", "yfinance", "akshare", "local"],
